...
export default function FormPortofolio() {
  ...
  const [typePortofolio, setTypePortofolio] = useState("")

  const handleSubmit = async (e: any) => {
    e.preventDefault();
    const res = await fetch("/api/workers/createPortofolio", {
      method: "POST",
      body: JSON.stringify({
        namaAplikasi,
        linkRepository,
          ...
          <label className="flex flex-col my-2 mx-2 text-[12px] text-[#9EA0A5]">
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              ...
              >
                <input
                  type="radio"
                  value={typePortofolio}
                  onChange={(e) => setTypePortofolio(e.target.value)}
